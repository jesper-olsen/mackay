...
def slplota(xQs, y, xt, pt, u, r, pausing, printing):
    plt.figure("slplota")
    plt.clf()
    ...
    if printing:
        plt.savefig(f'slice_step_{r}.png')
...
def slplotc(xQs, y, xt, pt, u, xnew, xleft, xright, pprop, r, pausing, specialprinting, logscale):
    plt.figure("slplotc")
    plt.clf()
...
def slplotd(xQs):
    ...
    if printing:
        plt.savefig(f'sliceplot_{len(xQs)}.pdf')

    plt.legend()
    plt.show()

def slice(pausing, xmin, xmax, mu, sigma, R, plotting, logscale):
    global printing, printrun
    defheight = -0.1
    xQs = []
    sumphi = 0.0
    ...
    for r in range(1, R + 1):
        ...
        if plotting == 1:
            slplota(np.array(xQs), y, xt, pt, u, r, pausing, printing)
        elif plotting == 2:
            ...
            slplotc(np.array(xQs), y, xt, pt, u, xnew, xleft, xright, pprop, r, pausing, False, logscale)
...
if __name__ == "__main__":
    ...
    logscale=False
    #print("slice sampling (5)") 
    #slice(True,-15,15,-0.9,1.5,5,2,logscale)  
    print("slice sampling (25)") 
    slice(True,-15,15,-0.9,1.5,25,2,logscale)  
